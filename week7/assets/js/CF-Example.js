// Courtesy of http://jsfiddle.net/x7y8d6m3/
// RB - Modified on 2020/02/22

function demo(presidents) {

    var ymdFormat = d3.time.format("%Y-%m-%d");
    // console.log("haha1 "+presidents[1].took_office);
    ...
    // console.log("haha "+presidents[1].took_office);
    ...
    var byParty = cf.dimension(p => p.party);



    console.log("-- group by party");
    var groupByParty = byParty.group();
    // "top" means how many instances to show
    groupByParty.top(Infinity).forEach(function(p,i) {
        console.log(p.key + ": " + p.value+"     i is "+ i);
    });
    ...
    //groupByParty update automatically
    groupByParty.top(Infinity).forEach(function(p, i) {
